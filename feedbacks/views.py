...
class FeedbackViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.groups.filter(name='SuperAdmin').exists():
            ...
            return Feedback.objects.filter(associate__care_home__admin=user)
        elif user.groups.filter(name='Manager').exists():
            managed_carehomes = CarehomeManagers.objects.filter(manager=user).values_list('carehome', flat=True)
            return Feedback.objects.filter(associate__care_home__in=managed_carehomes)
        else:
    ...
    def list(self, request, *args, **kwargs):
        ...
        associate_id = request.query_params.get('associate', None)
        start_date = request.query_params.get('start_date', None)
        end_date = request.query_params.get('end_date', None)

        if associate_id:
            queryset = queryset.filter(associate__id=associate_id)
